    ...
    <el-row :gutter="20">
      ...
      <el-col :span="20" :xs="24">
        <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
          ...
          <el-form-item label="手机号码" prop="employeeMobile">
            <el-input
        ...
        <el-row :gutter="10" class="mb8">
          ...
          <el-col :span="1.5">
            <el-button
              ...
              size="mini"
              @click="handleImport"
              v-hasPermi="['webadmin:employee:import']"
            >导入</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button
        ...
        <el-table v-loading="loading" :data="employeeList" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="用户编号" align="center" key="employeeId" prop="employeeId" v-if="columns[0].visible" />
          <el-table-column label="登录名" align="center" key="userName" prop="userName" v-if="columns[1].visible" :show-overflow-tooltip="true" />
          <el-table-column label="姓名" align="center" key="employeeName" prop="employeeName" v-if="columns[2].visible" :show-overflow-tooltip="true" />
          <el-table-column label="部门" align="center" key="deptName" prop="dept.deptName" v-if="columns[3].visible" :show-overflow-tooltip="true" />
          <el-table-column label="手机号码" align="center" key="employeeMobile" prop="employeeMobile" v-if="columns[4].visible" width="120" />
          <el-table-column label="状态" align="center" key="employeeStatus" v-if="columns[5].visible">
              ...
              <el-dropdown size="mini" @command="(command) => handleCommand(command, scope.row)" v-hasPermi="['webadmin:employee:resetPwd', 'webadmin:employee:edit']">
                ...
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item command="handleResetPwd" icon="el-icon-key"
                    v-hasPermi="['webadmin:employee:resetPwd']">重置密码</el-dropdown-item>
                  <el-dropdown-item command="handleAuthRole" icon="el-icon-circle-check"
                    v-hasPermi="['webadmin:employee:edit']">分配角色</el-dropdown-item>
                </el-dropdown-menu>
              </el-dropdown>
            </template>
          </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="手机号码" prop="employeeMobile">
              <el-input v-model="form.employeeMobile" placeholder="请输入手机号码" maxlength="11" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="邮箱" prop="employeeEmail">
              <el-input v-model="form.employeeEmail" placeholder="请输入邮箱" maxlength="50" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
            ...
            <el-form-item label="岗位">
              <el-select v-model="form.postIds" multiple placeholder="请选择岗位">
                <el-option
                  ...
                  :label="item.postName"
                  :value="item.postId"
                  :disabled="item.employeeStatus == 1"
                ></el-option>
              </el-select>
            </el-form-item>
          </el-col>
...
<script>
import { listEmployee, getEmployee, delEmployee, addEmployee, updateEmployee, resetEmployeePwd, changeEmployeeStatus, deptTreeSelect } from "@/api/webadmin/employee";
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/webadmin/employee/importData"
      },
      // 查询参数
      queryParams: {
        pageNum: 1,
  ...
  created() {
    this.getList();
    this.getDeptTree();
    this.getConfigKey("webadmin.employee.initPassword").then(response => {
  ...
  methods: {
    ...
    handleStatusChange(row) {
      let text = row.employeeStatus === "0" ? "启用" : "停用";
      this.$modal.confirm('确认要"' + text + '""' + row.userName + '"用户吗？').then(function() {
        return changeEmployeeStatus(row.employeeId, row.employeeStatus);
      }).then(() => {
    ...
    handleResetPwd(row) {
      this.$prompt('请输入"' + row.userName + '"的新密码', "提示", {
        ...
        inputErrorMessage: "用户密码长度必须介于 5 和 20 之间"
          ...
          resetUserPwd(row.employeeId, value).then(response => {
    ...
    handleAuthRole: function(row) {
      ...
      this.$router.push("/webadmin/employee-auth/role/" + employeeId);
    },
    /** 提交按钮 */
    submitForm: function() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除用户编号为"' + employeeIds + '"的数据项？').then(function() {
    ...
    /** 下载模板操作 */
    importTemplate() {
      this.download('webadmin/employee/importTemplate', {
